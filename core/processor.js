...
async function postGeminiWithRetry(history, shopConfig) {
  const maxAttempts = 3;
  let lastErr;
...
async function callGemini(userId, userMessage, shopConfig) {
  ...
  const res = await postGeminiWithRetry(history, shopConfig);
...
async function postFb(payload, attempts = 2, options = {}) {
  ...
  const token = options.fbPageToken || process.env.FB_PAGE_TOKEN;
  let lastErr;
...
async function sendMessage(recipientId, text, shopConfig) {
  ...
  for (const chunk of chunks) {
    ...
    }, 2, { fbPageToken: shopConfig?.credentials?.fbPageToken });
  }
}

async function sendImage(recipientId, imageUrl, shopConfig) {
  if (!imageUrl) return;
  await postFb({
    recipient: { id: recipientId },
    message: {
...
function showTyping(recipientId, shopConfig) {
  // Fire-and-forget: lỗi typing không chặn flow trả lời chính
  return postFb(
    { recipient: { id: recipientId }, sender_action: 'typing_on' },
    1,
    { timeout: 5000, fbPageToken: shopConfig?.credentials?.fbPageToken }
  ).catch(() => {});
}
...
async function sendTelegramAlert(leadData, shopConfig) {
  try {
    const botToken = shopConfig?.credentials?.telegramBotToken || process.env.TELEGRAM_BOT_TOKEN;
    const chatId = shopConfig?.credentials?.telegramChatId || process.env.TELEGRAM_CHAT_ID;
    if (!botToken || !chatId || !shopConfig?.features?.enableTelegram) return;

    const text = `🚨 CÓ ĐƠN HÀNG MỚI (Shop: ${shopConfig.name})!
...
async function handleMessage(shopConfig, messageData) {
  ...
  if (shouldSilenceAfterCompleteOrder(userText, senderId)) {
    ...
    if (justConfirmed) {
      ...
      sendTelegramAlert(confirmedLead, shopConfig);
      try {
        await sendMessage(senderId, buildDepositMessage(senderId), shopConfig);
        const qrUrl = getShopQrImageUrl(baseUrlOverride);
        if (qrUrl) {
          await sendImage(senderId, qrUrl, shopConfig);
  ...
  try {
    ...
    if (shouldUseFallbackReply(err)) {
      try {
        ...
        await sendMessage(senderId, fallback, shopConfig);
...
// Lấy ra handleMessage để dùng trong worker
module.exports = {
  handleMessage,
  storage,
  IMAGE_INDEX,
